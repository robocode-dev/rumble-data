...
Only accepted facts matching the current `engine.json` `behaviorVersion` contribute. The live leaderboard contains only catalog entries whose exact name and version are currently `active` and eligible for the selected game type. When a new version becomes active, it is a separate identity that starts at APS 0 with no samples; its superseded version disappears from the live ranking. Entries sort by descending APS with a stable bot-identity tie break.
