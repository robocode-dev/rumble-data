...
Aggregation deliberately owns no clock. `publication.py` hashes the complete ranking-visible `site/data/leaderboard/*.json` and `site/data/bots/*.json` tree and records that value in `site/data/history.json`. It advances `lastUpdatedAt` only when the regenerated hash differs from the last published hash, which also detects ranking data regenerated by a moderation pull request outside the publisher.
